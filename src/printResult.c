...
void printResult(Track t, float startFuel, float trackTemp, int safetyCar, int scStart){
    calculateStopNumber(t, startFuel, trackTemp, safetyCar, scStart);
    if(bestStrategy.fuelAmount <= 0.001){ // 0.001 is very low and very riskful for fuel amount
        ...
        printf("----------- STINTS -----------\n");
        for(int i = 0; stintsForTyre[i] != '\0'; i++){
            if(howManyAverage[i] == 0){
                howManyAverage[i] = t.totalLaps - totalLapsTaken;
            }
            printf("%d. stint: %c with ~%d laps\n", i + 1, stintsForTyre[i], howManyAverage[i]);
        }

        printf("-------------------------------------------------------------------------------------------\n");
        printf("Estimated total race time : %d min. %d sec.\n", bestStrategy.totalSeconds / 60, bestStrategy.totalSeconds % 60);
        printf("-------------------------------------------------------------------------------------------\n");
    }
